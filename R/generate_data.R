...
generate_data <- function(fparam, data_details, burnin = 100) {
  ...
  for (i in 1:length(list_names)) {
    ...
    data_tmp <- switch(process,
      bbridge = {
        results_tmp <- generate_brownian_bridge(
          N = N, v = fparam,
      ...
      bmotion = {
        results_tmp <- generate_brownian_motion(
          N = N, v = fparam,
          sd = data_details[[i]]$sd
        )
        results_tmp$data[, (N - N_data + 1):N]
      ...
      kl = {
        results <- generate_karhunen_loeve(
          Ns = N,
      ...
      ou = {
        results <- generate_ornstein_uhlenbeck(
          v = fparam, N = N,
      ...
      far1 = {
        ...
        results <- generate_far1(
          N = N, resolution = resolution1,
          sd = data_details[[i]]$sd,
          dependence = data_details[[i]]$dependence,
